...
@Component({
  selector: 'app-snow-particles',
  standalone: true,
  imports: [CommonModule],
  templateUrl: './snow-particles.component.html',
  styleUrl: './snow-particles.component.scss'
})
export class SnowParticlesComponent {
  snowflakes = Array(70).fill(0);

  getSnowflakePosition(index: number): string {
    const positions = [
  ...
  getSnowflakeDelay(index: number): string {
    const delays = [
  ...
  getSnowflakeDuration(index: number): string {
    const durations = [
  ...
  getSnowflakeSize(index: number): string {
    const sizes = [
